...
def get_user_analysis(username):
    ...
    try:
        ...
        print(f"• Teléfono: {user_info['phone'] or 'No registrado'}")
        ...
        if documents:
            for i, doc in enumerate(documents, 1):
                ...
                print(f"  • Tipo ID: {doc['doc_type_id']}")
        ...
        print(f"  • Documentos pendientes: {len([d for d in documents if d['status'] == 'PENDING'])}")
        print(f"  • Documentos aprobados: {len([d for d in documents if d['status'] == 'APPROVED'])}")
        print(f"  • Documentos rechazados: {len([d for d in documents if d['status'] == 'REJECTED'])}")
