...
def main():
    print("🔍 INICIANDO VERIFICACIÓN DE INTEGRIDAD DE DOCUMENTACIÓN (CORREGIDA)")
    print("=" * 70)
    ...
    for usuario in usuarios_inscritos:
        ...
        if usuario_tiene_problemas:
            usuarios_con_problemas.append({
                ...
                'telefono': telefono,
                'email': email,
                'documentos_ok': documentos_ok_usuario,
                'total_obligatorios': len(doc_types_obligatorios),
    ...
    csv_filename = f"usuarios_con_problemas_documentacion_corregido_{fecha_hora}.csv"
    
    with open(csv_filename, 'w', newline='', encoding='utf-8') as csvfile:
        fieldnames = ['nombre', 'apellido', 'dni', 'telefono', 'email', 'documentos_ok', 'total_obligatorios', 'problemas']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerows(usuarios_con_problemas)
    
    # Generar informe detallado
    informe_filename = f"informe_integridad_documentacion_corregido_{fecha_hora}.txt"
    
    with open(informe_filename, 'w', encoding='utf-8') as f:
        f.write("🔍 INFORME DE INTEGRIDAD DE DOCUMENTACIÓN MPD CONCURSOS (CORREGIDO)\n")
        f.write("=" * 70 + "\n\n")
        ...
        f.write(f"✅ Documentos OK: {documentos_ok}\n")
        ...
        # Calcular porcentajes por tipo de problema
        f.write("📊 DISTRIBUCIÓN DE PROBLEMAS:\n")
        f.write("-" * 30 + "\n")
        f.write(f"✅ Documentos íntegros: {documentos_ok} ({documentos_ok/total_documentos_verificados*100:.1f}%)\n")
        f.write(f"📋 Documentos faltantes: {documentos_faltantes} ({documentos_faltantes/total_documentos_verificados*100:.1f}%)\n")
        f.write(f"🗂️ Problemas físicos: {documentos_con_problemas_fisicos} ({documentos_con_problemas_fisicos/total_documentos_verificados*100:.1f}%)\n\n")
    ...
    print("🎯 RESUMEN FINAL DE VERIFICACIÓN (CORREGIDO)")
    print("=" * 70)
    ...
    print(f"📋 Documentos faltantes: {documentos_faltantes} ({documentos_faltantes/total_documentos_verificados*100:.1f}%)")
    print(f"🗂️ Documentos con problemas físicos: {documentos_con_problemas_fisicos} ({documentos_con_problemas_fisicos/total_documentos_verificados*100:.1f}%)")
    print()
