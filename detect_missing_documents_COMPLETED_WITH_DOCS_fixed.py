...
def main():
    ...
    if missing_files:
        ...
        # También guardamos info detallada para placeholders
        with open('missing_files_detailed_COMPLETED_WITH_DOCS.txt', 'w') as f:
            f.write("# Archivos faltantes con información detallada\n")
            f.write(f"# Generado: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write("# Formato: file_path|dni|full_name|document_type|document_status|upload_date\n\n")
            
            for doc in missing_files:
                f.write(f"{doc['file_path']}|{doc['dni']}|{doc['full_name']}|{doc['document_type']}|{doc['document_status']}|{doc['upload_date']}\n")
        
        print(f"💾 Información detallada guardada en: missing_files_detailed_COMPLETED_WITH_DOCS.txt")
    else:
