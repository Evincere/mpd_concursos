#!/bin/bash

# Base path donde están los documentos
BASE_PATH="/var/lib/docker/volumes/mpd_concursos_storage_data_prod/_data/documents"

echo "=== VERIFICACIÓN DE EXISTENCIA FÍSICA DE DOCUMENTOS ==="
echo "Ruta base: $BASE_PATH"
echo ""

# Contador de archivos encontrados y faltantes
found=0
missing=0
missing_files=()

# Obtener todas las rutas de documentos de usuarios COMPLETED_WITH_DOCS
docker exec -i mpd-concursos-mysql mysql -u root -proot1234 -D mpd_concursos -e "
SELECT DISTINCT d.file_path
FROM inscriptions i
JOIN user_entity u ON i.user_id = u.id
JOIN documents d ON d.user_id = u.id 
JOIN document_types dt ON d.document_type_id = dt.id
WHERE i.status = 'COMPLETED_WITH_DOCS'
    AND d.is_archived = 0
    AND d.file_path IS NOT NULL
ORDER BY d.file_path;
" 2>/dev/null | tail -n +2 | while read -r file_path; do
    if [ ! -z "$file_path" ]; then
        full_path="$BASE_PATH/$file_path"
        if [ -f "$full_path" ]; then
            found=$((found + 1))
            echo "✓ ENCONTRADO: $file_path"
        else
            missing=$((missing + 1))
            echo "✗ FALTANTE: $file_path"
            echo "$file_path" >> missing_files.tmp
        fi
    fi
done

echo ""
echo "=== RESUMEN ==="
echo "Archivos encontrados: $found"
echo "Archivos faltantes: $missing"

if [ -f missing_files.tmp ] && [ -s missing_files.tmp ]; then
    echo ""
    echo "=== ARCHIVOS FALTANTES ==="
    cat missing_files.tmp
    rm missing_files.tmp
fi
