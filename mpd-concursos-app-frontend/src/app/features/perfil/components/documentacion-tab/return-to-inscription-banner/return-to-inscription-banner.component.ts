...
export class ReturnToInscriptionBannerComponent implements OnInit, OnDestroy {
  ...
  returnToInscription(): void {
    // Obtener la inscripción guardada
    const inscription = this.inscriptionStateService.getInProgressInscription();
    const inscriptionId = this.inscriptionStateService.getRedirectFromInscription();

    console.log('[ReturnToInscriptionBanner] Intentando volver a la inscripción:', {
      inscriptionId,
    ...
    if (inscription && inscription.contestId) {
      ...
      // Esto abrirá automáticamente el diálogo de inscripción en el paso correcto
      ...
      // Si tenemos un ID pero no la inscripción completa, intentar navegar a concursos
      this.inscriptionStateService.clearRedirectFromInscription();

      // Redirigir al usuario a la página de concursos
      this.router.navigate(['/dashboard/concursos']);

      // Mostrar mensaje al usuario
      this.snackBar.open('Volviendo a la página de concursos...', 'Cerrar', {
        duration: 3000
      });
    } else {
