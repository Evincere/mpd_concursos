...
- **Sistema de almacenamiento unificado**: Implementación de estructura centralizada bajo `./storage/` para desarrollo y `/app/storage/` para producción
- **Configuración centralizada**: Nueva clase `StorageConfig` para gestionar todas las rutas de almacenamiento
- **Sistema completo de backups**: Scripts automáticos para backup de base de datos y archivos con rotación y notificaciones
- **Scripts de restauración**: Sistema completo de restauración con validaciones de seguridad
- **Configuración de cron**: Script para configurar backups automáticos programados

### 🔧 Mejoras Técnicas
- **Estructura unificada de carpetas**:
  - `storage/documents/` - Documentos MPD (DNI, CUIL, certificados)
  - `storage/contest-bases/` - Bases de concursos (PDFs)
  - `storage/cv-documents/` - Documentos de experiencia laboral y educación
  - `storage/profile-images/` - Imágenes de perfil de usuarios
  - `storage/temp/` - Archivos temporales

### 🛠️ Cambios en el Backend
- **StorageConfig**: Nueva clase de configuración centralizada con `@ConfigurationProperties`
- **FileStorageService**: Actualizado para usar StorageConfig
- **CvDocumentService**: Migrado al sistema unificado
- **FileSystemDocumentStorageService**: Refactorizado para nueva estructura
- **ProfileImageService**: Actualizado para usar configuración centralizada

### 📝 Configuración
- **application.properties**: Configuración unificada con variables centralizadas
- **Docker**: Volúmenes actualizados para usar `storage_data` y `backup_data`
- **.env.production**: Variables de entorno para almacenamiento unificado
- **.gitignore**: Actualizado para nueva estructura con exclusión de carpetas legacy

### 🔄 Scripts de Backup
- **backup-system.sh**: Script completo de backup con compresión y rotación
- **restore-system.sh**: Sistema de restauración con validaciones de seguridad
- **setup-backup-cron.sh**: Configuración automática de backups programados

### 🗂️ Migración
- **Sin migración requerida**: Implementación directa de estructura definitiva
- **Compatibilidad**: Mantenimiento de configuraciones legacy durante transición
- **Validación**: Compilación exitosa confirmada

## [Unreleased]

### Agregado
- **🔧 Sistema de Administración de Documentos Completamente Operativo**: Implementación completa sin mocks ni simulaciones
  - **Backend - AdminDocumentService**: Servicio completo para administración de documentos
    - Obtención de documentos con filtros avanzados y paginación server-side
    - Cálculo de estadísticas en tiempo real desde base de datos MySQL
    - Aprobación y rechazo de documentos con auditoría completa
    - JOIN con tabla de usuarios para información completa (nombre, email, DNI)
  - **Backend - AdminDocumentController**: Controlador REST con endpoints seguros
    - `GET /api/admin/documentos` - Listar documentos con filtros y paginación
    - `GET /api/admin/documentos/estadisticas` - Estadísticas en tiempo real
    ...
    - Logging completo para auditoría de acciones administrativas
  - **Frontend - Eliminación Completa de Mocks**: AdminDocumentosService actualizado
    - Reemplazado mock HttpClient con HttpClient real
    - Eliminados 40+ líneas de datos hardcodeados de ejemplo
    - Interfaces actualizadas para coincidir con respuestas del backend
    - Manejo de errores mejorado con LoggingService
  - **Frontend - DocumentosAdminComponent Operativo**: Componente completamente funcional
    - Carga de documentos desde base de datos real con paginación
    - Estadísticas calculadas en tiempo real (no localmente)
    - Filtros funcionales que consultan el backend
    - Aprobación/rechazo con actualización automática
    - Estados de carga y manejo de errores apropiados
